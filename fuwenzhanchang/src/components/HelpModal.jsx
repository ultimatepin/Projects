  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose}>
      <section className="help-modal" onMouseDown={(event) => event.stopPropagation()} aria-modal="true" role="dialog" aria-labelledby="help-title">
        ...
        <p className="help-lead">Only one device hosts the table. Device two must open the host’s full invite link—not its own localhost copy. Version 1.3.4 follows the official March 30, 2026 Core Rules for a best-of-one 1v1 Duel.</p>
        ...
        <div className="help-rules-note"><Swords /><span><strong>Cards first</strong>Your hand uses the available table width and wraps only when needed. Select a card for its legal inline actions; card costs automatically exhaust and recycle the required Runes.</span></div>
        <div className="help-rules-note"><BookOpen /><span><strong>How rules work in the app</strong>Universal rules—Runes, movement, showdowns, combat damage, Hold/Conquer scoring, burnout, and the 8-point strict-lead victory check—run on the host. Zaun Warrens also resolves its discard-then-draw Conquer trigger automatically or asks its controller when a choice is required. Resolve other printed card text together with the logged manual-effect controls because the bundled public catalog does not contain structured rules text, Champion tags, or Signature metadata.</span></div>
